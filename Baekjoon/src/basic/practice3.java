...
public static void main(String[] args) {
		...
		*/
		 ...
		 */
		 ...
		 /* 나의 풀이 
		테스트 케이스 만큼 반복문을 돌리면서 배열에 성적을 저장함과 동시에 누적 합을 같이 구해준다.
		...
		for(int i=0; i<testcase; i++) { // 테스트 케이스만큼 
			...
			score = new int[N]; // 학생 별 시험 점수 
			double sum = 0; // 성적 합 한번 돌 때마다 초기화 
			
			for(int j=0; j<N; j++){ // 성적 입력 
				score[j] = sc.nextInt(); 
				sum += score[j]; // 합계 
			}
			
			double avg = (sum/N); // 평균 
			double count = 0; // 평균 넘는 학생의 변수

			for(int j=0; j<N; j++) {
				if(score[j]>avg) {  // 평균 넘는 학생수 카운트 
					count++;
				}
			}
